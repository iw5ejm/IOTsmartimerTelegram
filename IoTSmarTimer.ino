...
#define      Version           "1.06"               // Firmware version
...
// I/O config
//-----------
//#define      Relay             3   // Output relay (URXD)
//#define      Button            2   // Button (GPIO2)
//#define      LED               0   // Status LED (GPIO0)
//#define      Spare             1   // not used

#define      Relay             0   // Output relay gpio0
#define      Button            3   // Button rxpin
#define      LED               1   // Status LED txpin
#define      Spare             2   // not used gpio02

// Variables
//----------
#define      ErrMsg            "<font color=\"red\"> < < Error !</font>"
#define      EEPROM_chk        100
  ...
  if ( (DataAvailable == true) and (request.indexOf("/") != -1) ) {
    ...
    if ( (TimeOk == false) and (Mode == 2) ) { 
      ...
      if ( (ManualTimeOn == true) and (Mode == 2) ){
        client.print("ON for ");
        if ( (ManualSecOn/3600) > 0) {
          client.print((ManualSecOn/3600));
          client.print(" hours, ");
        }
        client.print( ( (ManualSecOn%3600)/60) );
        client.print(" minutes");
      }

      if ( (ManualTimeOff == true) and (Mode == 2) ){
        client.print("Standby for ");
        if ( (ManualSecOff/3600) > 0) {
          client.print((ManualSecOff/3600));
          client.print(" hours, ");
        }
        client.print( ( (ManualSecOff%3600)/60) );
        client.print(" minutes");
      }
     
      if ( (ManualOff == false) and (ManualOn == false) and (ManualTimeOff == false) and (ManualTimeOn == false) ) {
        if (digitalRead(Relay) == 0) client.print("ON"); else client.print("OFF");
      }
    }
